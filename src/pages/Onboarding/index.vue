<template>
  <q-page class="container">
    <div class="onboarding__base text-gray">
      <div class="onboarding__center" v-if="shouldShow('user-setup')">
        <user-setup-regular
          @next="showProcessSetup"
          @skipFNStep="skipFNStep"/>
      </div>
      <process-setup :userName="userName"
                    :skipScore="shouldSkipScore"
                    :emailAddress="currentUser.email"
                    @finish="finishOnboardingClicked"
                    @start-over="showType = 'user-setup'"
                    v-else-if="shouldShow('process-setup')"/>
    </div>
  </q-page>
</template>
...
export default {
  components: {
    ...
    'process-setup': () => import('@/components/onboarding/_process-setup')
  },
  computed: {
    ...mapGetters({
  ...
  methods: {
    ...
    finishOnboardingClicked () {
      ...
      this.$router.push('/training-plan?onboarding=first')
    },
